...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        targets = torch.stack((1.0-targets, targets), dim=1)

        inputs_flat = inputs.view(N, -1)
        targets_flat = targets.view(N, -1).type(torch.float32)

        intersection = inputs_flat * targets_flat
 
        loss = 2 * (intersection.sum(1) + smooth) / (inputs_flat.sum(1) + targets_flat.sum(1) + smooth)
        loss = 1 - loss.sum() / N
...
# added by ying
class MixedLovaszCrossEntropyLoss(nn.Module):

    def __init__(self, weight=None):
        super().__init__()
        self.lovasz = LovaszLoss2d()
        self.ce = CrossEntropyLoss2d(weight)


    def forward(self, inputs, targets, lovasz_weight=0.5, ce_weight=0.5):
        lovasz_loss = self.lovasz(inputs, targets)
        ce_loss = self.ce(inputs,targets)

        return lovasz_weight*lovasz_loss + ce_weight*ce_loss
