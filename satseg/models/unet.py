...
class ConvRelu(nn.Module):
    ...
    def __init__(self, num_in, num_out):
        ...
        self.inorm = nn.InstanceNorm2d(num_out)
...
class UNet(nn.Module):
    ...
    def __init__(self, backbone_name='resnet101', num_classes=2, num_filters=32, pretrained=True):
    ...
    def forward(self, x):
        ...
        enc0 = self.backbone.conv1(x)
        enc0 = self.backbone.bn1(enc0)
        enc0 = self.backbone.relu(enc0)
        enc0 = self.backbone.maxpool(enc0)

        enc1 = self.backbone.layer1(enc0)
        enc2 = self.backbone.layer2(enc1)
        enc3 = self.backbone.layer3(enc2)
        enc4 = self.backbone.layer4(enc3)
